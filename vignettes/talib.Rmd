...
Real-world data often contains missing values. By default (`na.ignore = FALSE`), any `NA` in the input propagates through the entire calculation, which can fill the result with `NA`s.

Setting `na.ignore = TRUE` strips `NA` rows before calculation, computes the indicator on the clean data, and then re-inserts `NA`s at their original positions:
