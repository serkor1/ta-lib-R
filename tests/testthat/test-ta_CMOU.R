...
## unsmoothed_chande_momentum_oscillator and CMOU produces the same results
testthat::test_that(desc = 'Alias and function similarity', code = {
	## 1) test that the alias and
	##    function returns the same values
	output <- unsmoothed_chande_momentum_oscillator(SPY)
	alias <- CMOU(SPY)
	...
	camel <- unsmoothedChandeMomentumOscillator(SPY)
...
testthat::test_that(desc = 'Class in, class out (<matrix>)', code = {
	...
	testthat::expect_true(
		inherits(unsmoothed_chande_momentum_oscillator(SPY), class(SPY))
	)
})
...
testthat::test_that(desc = 'Class in, class out (<data.frame>)', code = {
	...
	##    matches the input class
	testthat::expect_true(
		inherits(unsmoothed_chande_momentum_oscillator(BTC), class(BTC))
	)
})
...
testthat::test_that(desc = 'Class in, class out (<xts>)', code = {
	...
	##    matches the input class
	testthat::expect_true(
		inherits(unsmoothed_chande_momentum_oscillator(GOOGL), class(GOOGL))
	)
})
...
testthat::test_that(desc = 'Value parity with <data.frame> (<xts>)', code = {
	...
	reference <- unname(as.matrix(unsmoothed_chande_momentum_oscillator(
		reference
	)))

	testthat::expect_equal(
		object = unname(zoo::coredata(unsmoothed_chande_momentum_oscillator(
			GOOGL
		))),
		expected = reference
	)
	...
	testthat::expect_equal(
		object = unname(zoo::coredata(unsmoothed_chande_momentum_oscillator(GOOGL[, sort(colnames(
			GOOGL
		))]))),
		expected = reference
	)
...
testthat::test_that(desc = 'Default calls', code = {
	testthat::expect_equal(
		object = unsmoothed_chande_momentum_oscillator(
			BTC
		),
		expected = unsmoothed_chande_momentum_oscillator(
			BTC,
			cols = ~close
		)
	)
...
testthat::test_that(desc = 'Equal length of input and output for <data.frame> with na.bridge = TRUE', code = {
	testthat::expect_equal(
		object = nrow(unsmoothed_chande_momentum_oscillator(
			ATOM,
			na.bridge = TRUE
		)),
		expected = nrow(ATOM)
...
testthat::test_that(desc = 'Row names are respected for <data.frame>, na.bridge = TRUE', code = {
	...
	## calculate indicator
	indicator <- unsmoothed_chande_momentum_oscillator(ATOM, na.bridge = TRUE)
...
testthat::test_that(desc = 'Row names are respected for <data.frame>', code = {
	...
	## calculate indicator
	indicator <- unsmoothed_chande_momentum_oscillator(BTC)
...
testthat::test_that(desc = 'Row names are respected for <matrix>', code = {
	...
	## calculate indicator
	indicator <- unsmoothed_chande_momentum_oscillator(SPY)
...
testthat::test_that(desc = 'Lookback equivalence', code = {
	output <- attr(
		unsmoothed_chande_momentum_oscillator(x = SPY),
		"lookback"
	)
	...
	testthat::expect_equal(
		object = output,
		expected = lookback(
			FUN = unsmoothed_chande_momentum_oscillator,
			x = SPY
		)
	)
...
testthat::test_that(desc = '<plotly>-methods for <data.frame>', code = {
	## check that unsmoothed_chande_momentum_oscillator can
	## use <plotly> without any issues
	output <- testthat::expect_no_error(
		{
			chart(BTC)
			indicator(unsmoothed_chande_momentum_oscillator)
		}
	)
...
testthat::test_that(desc = '<numeric> methods', code = {
	...
	x <- testthat::expect_no_condition(
		unsmoothed_chande_momentum_oscillator(BTC[[1]])
	)
