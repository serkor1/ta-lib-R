...
## relative_strength_index and RSI produces the same results
testthat::test_that(desc = 'Alias and function similarity', code = {
	...
	camel <- relativeStrengthIndex(SPY)
...
testthat::test_that(desc = 'Class in, class out (<xts>)', code = {
	...
	##    matches the input class
	testthat::expect_true(
		inherits(relative_strength_index(GOOGL), class(GOOGL))
	)
})
...
testthat::test_that(desc = 'Value parity with <data.frame> (<xts>)', code = {
	...
	reference <- unname(as.matrix(relative_strength_index(reference)))

	testthat::expect_equal(
		object = unname(zoo::coredata(relative_strength_index(GOOGL))),
		expected = reference
	)
	...
	testthat::expect_equal(
		object = unname(zoo::coredata(relative_strength_index(GOOGL[, sort(colnames(
			GOOGL
		))]))),
		expected = reference
	)
...
testthat::test_that(desc = 'Equal length of input and output for <data.frame> with na.bridge = TRUE', code = {
	testthat::expect_equal(
		object = nrow(relative_strength_index(
			ATOM,
			na.bridge = TRUE
		)),
		expected = nrow(ATOM)
...
testthat::test_that(desc = 'Row names are respected for <data.frame>, na.bridge = TRUE', code = {
	...
	## calculate indicator
	indicator <- relative_strength_index(ATOM, na.bridge = TRUE)
...
testthat::test_that(desc = 'Row names are respected for <data.frame>', code = {
	...
	## calculate indicator
	indicator <- relative_strength_index(BTC)
...
testthat::test_that(desc = 'Row names are respected for <matrix>', code = {
	...
	## calculate indicator
	indicator <- relative_strength_index(SPY)
...
testthat::test_that(desc = 'Lookback equivalence', code = {
	output <- attr(
		relative_strength_index(x = SPY),
		"lookback"
	)

	testthat::expect_equal(
		object = output,
		expected = lookback(FUN = relative_strength_index, x = SPY)
	)

	## the camelCase lookback alias
	## is the function itself
	testthat::expect_identical(
		object = talib:::relativeStrengthIndex_lookback,
		expected = talib:::relative_strength_index_lookback
	)
})
...
testthat::test_that(desc = '<plotly>-methods for <data.frame>', code = {
	## check that relative_strength_index can
