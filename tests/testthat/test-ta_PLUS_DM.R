...
testthat::test_that(desc = 'Alias and function similarity', code = {
	...
	camel <- plusDirectionalMovement(SPY)
...
testthat::test_that(desc = 'Class in, class out (<xts>)', code = {
	...
	##    matches the input class
	testthat::expect_true(
		inherits(plus_directional_movement(GOOGL), class(GOOGL))
	)
})
...
testthat::test_that(desc = 'Value parity with <data.frame> (<xts>)', code = {
	...
	reference <- unname(as.matrix(plus_directional_movement(reference)))

	testthat::expect_equal(
		object = unname(zoo::coredata(plus_directional_movement(GOOGL))),
		expected = reference
	)
	...
	testthat::expect_equal(
		object = unname(zoo::coredata(plus_directional_movement(GOOGL[, sort(colnames(
			GOOGL
		))]))),
		expected = reference
	)
...
testthat::test_that(desc = 'Row names are respected for <data.frame>, na.bridge = TRUE', code = {
	...
	## calculate indicator
	indicator <- plus_directional_movement(ATOM, na.bridge = TRUE)
...
testthat::test_that(desc = 'Row names are respected for <data.frame>', code = {
	...
	## calculate indicator
	indicator <- plus_directional_movement(BTC)
...
testthat::test_that(desc = 'Row names are respected for <matrix>', code = {
	...
	## calculate indicator
	indicator <- plus_directional_movement(SPY)
...
testthat::test_that(desc = 'Lookback equivalence', code = {
	output <- attr(
		plus_directional_movement(x = SPY),
		"lookback"
	)

	testthat::expect_equal(
		object = output,
		expected = lookback(FUN = plus_directional_movement, x = SPY)
	)

	## the camelCase lookback alias
	## is the function itself
	testthat::expect_identical(
		object = talib:::plusDirectionalMovement_lookback,
		expected = talib:::plus_directional_movement_lookback
	)
})
