...
testthat::test_that(desc = 'Alias and function similarity', code = {
	...
	camel <- averageDirectionalMovementIndex(SPY)
...
testthat::test_that(desc = 'Class in, class out (<matrix>)', code = {
	...
	testthat::expect_true(
		inherits(average_directional_movement_index(SPY), class(SPY))
	)
})
...
testthat::test_that(desc = 'Class in, class out (<data.frame>)', code = {
	...
	##    matches the input class
	testthat::expect_true(
		inherits(average_directional_movement_index(BTC), class(BTC))
	)
})
...
testthat::test_that(desc = 'Value parity with <data.frame> (<xts>)', code = {
	...
	reference <- unname(as.matrix(average_directional_movement_index(
		reference
	)))

	testthat::expect_equal(
		object = unname(zoo::coredata(average_directional_movement_index(
			GOOGL
		))),
		expected = reference
	)
	...
	testthat::expect_equal(
		object = unname(zoo::coredata(average_directional_movement_index(GOOGL[, sort(colnames(
			GOOGL
		))]))),
		expected = reference
	)
...
testthat::test_that(desc = 'Default calls', code = {
	testthat::expect_equal(
		object = average_directional_movement_index(
			BTC
		),
		expected = average_directional_movement_index(
			BTC,
			cols = ~ high + low + close
		)
	)
...
testthat::test_that(desc = 'Equal length of input and output for <data.frame> with na.bridge = TRUE', code = {
	testthat::expect_equal(
		object = nrow(average_directional_movement_index(
			ATOM,
			na.bridge = TRUE
		)),
		expected = nrow(ATOM)
...
testthat::test_that(desc = 'Row names are respected for <data.frame>, na.bridge = TRUE', code = {
	...
	## calculate indicator
	indicator <- average_directional_movement_index(ATOM, na.bridge = TRUE)
...
testthat::test_that(desc = 'Row names are respected for <data.frame>', code = {
	...
	## calculate indicator
	indicator <- average_directional_movement_index(BTC)
...
testthat::test_that(desc = 'Row names are respected for <matrix>', code = {
	...
	## calculate indicator
	indicator <- average_directional_movement_index(SPY)
...
testthat::test_that(desc = 'Lookback equivalence', code = {
	output <- attr(
		average_directional_movement_index(x = SPY),
		"lookback"
	)

	testthat::expect_equal(
		object = output,
		expected = lookback(FUN = average_directional_movement_index, x = SPY)
	)
...
testthat::test_that(desc = '<plotly>-methods for <data.frame>', code = {
	## check that average_directional_movement_index can
