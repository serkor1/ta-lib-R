...
testthat::test_that(desc = 'Alias and function similarity', code = {
	...
	camel <- parabolicStopAndReverse(SPY)
...
testthat::test_that(desc = 'Class in, class out (<matrix>)', code = {
	...
	testthat::expect_true(
		inherits(parabolic_stop_and_reverse(SPY), class(SPY))
	)
})
...
testthat::test_that(desc = 'Class in, class out (<data.frame>)', code = {
	...
	##    matches the input class
	testthat::expect_true(
		inherits(parabolic_stop_and_reverse(BTC), class(BTC))
	)
})
...
testthat::test_that(desc = 'Class in, class out (<xts>)', code = {
	...
	##    matches the input class
	testthat::expect_true(
		inherits(parabolic_stop_and_reverse(GOOGL), class(GOOGL))
	)
})
...
testthat::test_that(desc = 'Value parity with <data.frame> (<xts>)', code = {
	...
	reference <- unname(as.matrix(parabolic_stop_and_reverse(reference)))

	testthat::expect_equal(
		object = unname(zoo::coredata(parabolic_stop_and_reverse(GOOGL))),
		expected = reference
	)
	...
	testthat::expect_equal(
		object = unname(zoo::coredata(parabolic_stop_and_reverse(GOOGL[, sort(colnames(
			GOOGL
		))]))),
		expected = reference
	)
...
testthat::test_that(desc = 'Default calls', code = {
	testthat::expect_equal(
		object = parabolic_stop_and_reverse(
			BTC
		),
		expected = parabolic_stop_and_reverse(
			BTC,
			cols = ~ high + low
		)
	)
...
testthat::test_that(desc = 'Equal length of input and output for <data.frame> with na.bridge = TRUE', code = {
	testthat::expect_equal(
		object = nrow(parabolic_stop_and_reverse(
			ATOM,
			na.bridge = TRUE
		)),
		expected = nrow(ATOM)
...
testthat::test_that(desc = 'Row names are respected for <data.frame>, na.bridge = TRUE', code = {
	...
	## calculate indicator
	indicator <- parabolic_stop_and_reverse(ATOM, na.bridge = TRUE)
...
testthat::test_that(desc = 'Row names are respected for <data.frame>', code = {
	...
	## calculate indicator
	indicator <- parabolic_stop_and_reverse(BTC)
...
testthat::test_that(desc = 'Row names are respected for <matrix>', code = {
	...
	## calculate indicator
	indicator <- parabolic_stop_and_reverse(SPY)
...
testthat::test_that(desc = 'Lookback equivalence', code = {
	output <- attr(
		parabolic_stop_and_reverse(x = SPY),
		"lookback"
	)

	testthat::expect_equal(
		object = output,
		expected = lookback(FUN = parabolic_stop_and_reverse, x = SPY)
	)

	## the camelCase lookback alias
	## is the function itself
	testthat::expect_identical(
		object = talib:::parabolicStopAndReverse_lookback,
		expected = talib:::parabolic_stop_and_reverse_lookback
	)
})
...
testthat::test_that(desc = '<plotly>-methods for <data.frame>', code = {
	## check that parabolic_stop_and_reverse can
	## use <plotly> without any issues
	output <- testthat::expect_no_error(
		{
			chart(BTC)
			indicator(parabolic_stop_and_reverse)
		}
	)
