...
## mesa_adaptive_moving_average and MAMA produces the same results
testthat::test_that(desc = 'Alias and function similarity', code = {
	...
	camel <- mesaAdaptiveMovingAverage(SPY)
...
testthat::test_that(desc = 'Class in, class out (<xts>)', code = {
	...
	##    matches the input class
	testthat::expect_true(
		inherits(mesa_adaptive_moving_average(GOOGL), class(GOOGL))
	)
})
...
testthat::test_that(desc = 'Value parity with <data.frame> (<xts>)', code = {
	...
	reference <- unname(as.matrix(mesa_adaptive_moving_average(reference)))

	testthat::expect_equal(
		object = unname(zoo::coredata(mesa_adaptive_moving_average(GOOGL))),
		expected = reference
	)
	...
	testthat::expect_equal(
		object = unname(zoo::coredata(mesa_adaptive_moving_average(GOOGL[, sort(colnames(
			GOOGL
		))]))),
		expected = reference
	)
...
testthat::test_that(desc = 'Equal length of input and output for <data.frame> with na.bridge = TRUE', code = {
	testthat::expect_equal(
		object = nrow(mesa_adaptive_moving_average(
			ATOM,
			na.bridge = TRUE
		)),
		expected = nrow(ATOM)
...
testthat::test_that(desc = 'Row names are respected for <data.frame>, na.bridge = TRUE', code = {
	...
	## calculate indicator
	indicator <- mesa_adaptive_moving_average(ATOM, na.bridge = TRUE)
...
testthat::test_that(desc = 'Row names are respected for <data.frame>', code = {
	...
	## calculate indicator
	indicator <- mesa_adaptive_moving_average(BTC)
...
testthat::test_that(desc = 'Row names are respected for <matrix>', code = {
	...
	## calculate indicator
	indicator <- mesa_adaptive_moving_average(SPY)
...
testthat::test_that(desc = 'Lookback equivalence', code = {
	output <- attr(
		mesa_adaptive_moving_average(x = SPY),
		"lookback"
	)

	testthat::expect_equal(
		object = output,
		expected = lookback(FUN = mesa_adaptive_moving_average, x = SPY)
	)

	## the camelCase lookback alias
	## is the function itself
	testthat::expect_identical(
		object = talib:::mesaAdaptiveMovingAverage_lookback,
		expected = talib:::mesa_adaptive_moving_average_lookback
	)
})
...
testthat::test_that(desc = '<plotly>-methods for <data.frame>', code = {
	## check that mesa_adaptive_moving_average can
