...
## average_directional_movement_index_rating and ADXR produces the same results
testthat::test_that(desc = 'Alias and function similarity', code = {
	## 1) test that the alias and
	##    function returns the same values
	output <- average_directional_movement_index_rating(SPY)
	alias <- ADXR(SPY)
	...
	camel <- averageDirectionalMovementIndexRating(SPY)
...
testthat::test_that(desc = 'Class in, class out (<matrix>)', code = {
	...
	testthat::expect_true(
		inherits(average_directional_movement_index_rating(SPY), class(SPY))
	)
})
...
testthat::test_that(desc = 'Class in, class out (<data.frame>)', code = {
	...
	##    matches the input class
	testthat::expect_true(
		inherits(average_directional_movement_index_rating(BTC), class(BTC))
	)
})
...
testthat::test_that(desc = 'Class in, class out (<xts>)', code = {
	...
	##    matches the input class
	testthat::expect_true(
		inherits(average_directional_movement_index_rating(GOOGL), class(GOOGL))
	)
})
...
testthat::test_that(desc = 'Value parity with <data.frame> (<xts>)', code = {
	...
	reference <- unname(as.matrix(average_directional_movement_index_rating(
		reference
	)))

	testthat::expect_equal(
		object = unname(zoo::coredata(average_directional_movement_index_rating(
			GOOGL
		))),
		expected = reference
	)
	...
	testthat::expect_equal(
		object = unname(zoo::coredata(average_directional_movement_index_rating(GOOGL[, sort(colnames(
			GOOGL
		))]))),
		expected = reference
	)
...
testthat::test_that(desc = 'Default calls', code = {
	testthat::expect_equal(
		object = average_directional_movement_index_rating(
			BTC
		),
		expected = average_directional_movement_index_rating(
			BTC,
			cols = ~ high + low + close
		)
	)
...
testthat::test_that(desc = 'Equal length of input and output for <data.frame> with na.bridge = TRUE', code = {
	testthat::expect_equal(
		object = nrow(average_directional_movement_index_rating(
			ATOM,
			na.bridge = TRUE
		)),
		expected = nrow(ATOM)
...
testthat::test_that(desc = 'Row names are respected for <data.frame>, na.bridge = TRUE', code = {
	...
	## calculate indicator
	indicator <- average_directional_movement_index_rating(
		ATOM,
		na.bridge = TRUE
	)
...
testthat::test_that(desc = 'Row names are respected for <data.frame>', code = {
	...
	## calculate indicator
	indicator <- average_directional_movement_index_rating(BTC)
...
testthat::test_that(desc = 'Row names are respected for <matrix>', code = {
	...
	## calculate indicator
	indicator <- average_directional_movement_index_rating(SPY)
...
testthat::test_that(desc = 'Lookback equivalence', code = {
	output <- attr(
		average_directional_movement_index_rating(x = SPY),
		"lookback"
	)
	...
	testthat::expect_identical(
		object = talib:::averageDirectionalMovementIndexRating_lookback,
		expected = talib:::average_directional_movement_index_rating_lookback
	)
})
...
testthat::test_that(desc = '<plotly>-methods for <data.frame>', code = {
	## check that average_directional_movement_index_rating can
