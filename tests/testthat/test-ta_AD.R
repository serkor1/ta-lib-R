...
## chaikin_accumulation_distribution_line and AD produces the same results
testthat::test_that(desc = 'Alias and function similarity', code = {
	## 1) test that the alias and
	##    function returns the same values
	output <- chaikin_accumulation_distribution_line(SPY)
	alias <- AD(SPY)
	...
	camel <- chaikinAccumulationDistributionLine(SPY)
...
testthat::test_that(desc = 'Class in, class out (<matrix>)', code = {
	...
	testthat::expect_true(
		inherits(chaikin_accumulation_distribution_line(SPY), class(SPY))
	)
})
...
testthat::test_that(desc = 'Class in, class out (<data.frame>)', code = {
	...
	##    matches the input class
	testthat::expect_true(
		inherits(chaikin_accumulation_distribution_line(BTC), class(BTC))
	)
})
...
testthat::test_that(desc = 'Class in, class out (<xts>)', code = {
	...
	##    matches the input class
	testthat::expect_true(
		inherits(chaikin_accumulation_distribution_line(GOOGL), class(GOOGL))
	)
})
...
testthat::test_that(desc = 'Value parity with <data.frame> (<xts>)', code = {
	...
	reference <- unname(as.matrix(chaikin_accumulation_distribution_line(
		reference
	)))

	testthat::expect_equal(
		object = unname(zoo::coredata(chaikin_accumulation_distribution_line(
			GOOGL
		))),
		expected = reference
	)
	...
	testthat::expect_equal(
		object = unname(zoo::coredata(chaikin_accumulation_distribution_line(GOOGL[, sort(colnames(
			GOOGL
		))]))),
		expected = reference
	)
...
testthat::test_that(desc = 'Default calls', code = {
	testthat::expect_equal(
		object = chaikin_accumulation_distribution_line(
			BTC
		),
		expected = chaikin_accumulation_distribution_line(
			BTC,
...
testthat::test_that(desc = 'Equal length of input and output for <data.frame> with na.bridge = TRUE', code = {
	testthat::expect_equal(
		object = nrow(chaikin_accumulation_distribution_line(
			ATOM,
			na.bridge = TRUE
		)),
		expected = nrow(ATOM)
...
testthat::test_that(desc = 'Row names are respected for <data.frame>, na.bridge = TRUE', code = {
	...
	## calculate indicator
	indicator <- chaikin_accumulation_distribution_line(ATOM, na.bridge = TRUE)
...
testthat::test_that(desc = 'Row names are respected for <data.frame>', code = {
	...
	## calculate indicator
	indicator <- chaikin_accumulation_distribution_line(BTC)
...
testthat::test_that(desc = 'Row names are respected for <matrix>', code = {
	...
	## calculate indicator
	indicator <- chaikin_accumulation_distribution_line(SPY)
...
testthat::test_that(desc = 'Lookback equivalence', code = {
	output <- attr(
		chaikin_accumulation_distribution_line(x = SPY),
		"lookback"
	)
	...
	testthat::expect_identical(
		object = talib:::chaikinAccumulationDistributionLine_lookback,
		expected = talib:::chaikin_accumulation_distribution_line_lookback
	)
})
...
testthat::test_that(desc = '<plotly>-methods for <data.frame>', code = {
	## check that chaikin_accumulation_distribution_line can
	## use <plotly> without any issues
	output <- testthat::expect_no_error(
		{
			chart(BTC)
			indicator(chaikin_accumulation_distribution_line)
		}
	)
