...
testthat::test_that(desc = 'Alias and function similarity', code = {
	...
	camel <- minusDirectionalMovement(SPY)
...
testthat::test_that(desc = 'Class in, class out (<xts>)', code = {
	...
	##    matches the input class
	testthat::expect_true(
		inherits(minus_directional_movement(GOOGL), class(GOOGL))
	)
})
...
testthat::test_that(desc = 'Value parity with <data.frame> (<xts>)', code = {
	...
	reference <- unname(as.matrix(minus_directional_movement(reference)))

	testthat::expect_equal(
		object = unname(zoo::coredata(minus_directional_movement(GOOGL))),
		expected = reference
	)
	...
	testthat::expect_equal(
		object = unname(zoo::coredata(minus_directional_movement(GOOGL[, sort(colnames(
			GOOGL
		))]))),
		expected = reference
	)
...
testthat::test_that(desc = 'Row names are respected for <data.frame>, na.bridge = TRUE', code = {
	...
	## calculate indicator
	indicator <- minus_directional_movement(ATOM, na.bridge = TRUE)
...
testthat::test_that(desc = 'Row names are respected for <data.frame>', code = {
	...
	## calculate indicator
	indicator <- minus_directional_movement(BTC)
...
testthat::test_that(desc = 'Row names are respected for <matrix>', code = {
	...
	## calculate indicator
	indicator <- minus_directional_movement(SPY)
...
testthat::test_that(desc = 'Lookback equivalence', code = {
	output <- attr(
		minus_directional_movement(x = SPY),
		"lookback"
	)

	testthat::expect_equal(
		object = output,
		expected = lookback(FUN = minus_directional_movement, x = SPY)
	)
	...
	testthat::expect_identical(
		...
		expected = talib:::minus_directional_movement_lookback
	)
})
...
testthat::test_that(desc = '<plotly>-methods for <data.frame>', code = {
	## check that minus_directional_movement can
