...
testthat::test_that(desc = "Stochastic Relative Strength Index", code = {
	...
	output <- stochastic_relative_strength_index(SPY)
	alias <- STOCHRSI(SPY)
	...
	output <- testthat::expect_no_error(
		{
			chart(BTC)
			indicator(stochastic_relative_strength_index)
		}
	)
	...
	## 3.1) matrix
	testthat::expect_true(
		inherits(stochastic_relative_strength_index(SPY), class(SPY))
	)

	## 3.2) data.frame
	testthat::expect_true(
		inherits(stochastic_relative_strength_index(BTC), class(BTC))
	)
