...
testthat::test_that(desc = 'Alias and function similarity', code = {
	...
	camel <- fixedMovingAverageConvergenceDivergence(SPY)
...
testthat::test_that(desc = 'Class in, class out (<data.frame>)', code = {
	...
	##    matches the input class
	testthat::expect_true(
		inherits(fixed_moving_average_convergence_divergence(BTC), class(BTC))
	)
})
...
testthat::test_that(desc = 'Class in, class out (<xts>)', code = {
	...
	testthat::expect_true(
		inherits(
			fixed_moving_average_convergence_divergence(GOOGL),
			class(GOOGL)
		)
	)
})
...
testthat::test_that(desc = 'Value parity with <data.frame> (<xts>)', code = {
	...
	reference <- unname(as.matrix(fixed_moving_average_convergence_divergence(
		reference
	)))

	testthat::expect_equal(
		object = unname(zoo::coredata(fixed_moving_average_convergence_divergence(
			GOOGL
		))),
		expected = reference
	)
	...
	testthat::expect_equal(
		object = unname(zoo::coredata(fixed_moving_average_convergence_divergence(GOOGL[, sort(colnames(
			GOOGL
		))]))),
		expected = reference
	)
...
testthat::test_that(desc = 'Default calls', code = {
	testthat::expect_equal(
		...
		expected = fixed_moving_average_convergence_divergence(
			BTC,
			cols = ~close
		)
	)
...
testthat::test_that(desc = 'Lookback equivalence', code = {
	...
	testthat::expect_identical(
		object = talib:::fixedMovingAverageConvergenceDivergence_lookback,
...
testthat::test_that(desc = '<plotly>-methods for <data.frame>', code = {
	## check that fixed_moving_average_convergence_divergence can
