...
testthat::test_that(desc = 'Alias and function similarity', code = {
	...
	camel <- normalizedAverageTrueRange(SPY)
...
testthat::test_that(desc = 'Class in, class out (<matrix>)', code = {
	...
	testthat::expect_true(
		inherits(normalized_average_true_range(SPY), class(SPY))
	)
})
...
testthat::test_that(desc = 'Class in, class out (<data.frame>)', code = {
	...
	##    matches the input class
	testthat::expect_true(
		inherits(normalized_average_true_range(BTC), class(BTC))
	)
})
...
testthat::test_that(desc = 'Value parity with <data.frame> (<xts>)', code = {
	...
	reference <- unname(as.matrix(normalized_average_true_range(reference)))

	testthat::expect_equal(
		object = unname(zoo::coredata(normalized_average_true_range(GOOGL))),
		expected = reference
	)
	...
	testthat::expect_equal(
		object = unname(zoo::coredata(normalized_average_true_range(GOOGL[, sort(colnames(
			GOOGL
		))]))),
		expected = reference
	)
...
testthat::test_that(desc = 'Default calls', code = {
	testthat::expect_equal(
		object = normalized_average_true_range(
			BTC
		),
		expected = normalized_average_true_range(
			BTC,
			cols = ~ high + low + close
		)
	)
...
testthat::test_that(desc = 'Equal length of input and output for <data.frame> with na.bridge = TRUE', code = {
	testthat::expect_equal(
		object = nrow(normalized_average_true_range(
			ATOM,
			na.bridge = TRUE
		)),
		expected = nrow(ATOM)
...
testthat::test_that(desc = 'Row names are respected for <data.frame>, na.bridge = TRUE', code = {
	...
	## calculate indicator
	indicator <- normalized_average_true_range(ATOM, na.bridge = TRUE)
...
testthat::test_that(desc = 'Row names are respected for <data.frame>', code = {
	...
	## calculate indicator
	indicator <- normalized_average_true_range(BTC)
...
testthat::test_that(desc = 'Row names are respected for <matrix>', code = {
	...
	## calculate indicator
	indicator <- normalized_average_true_range(SPY)
...
testthat::test_that(desc = 'Lookback equivalence', code = {
	output <- attr(
		normalized_average_true_range(x = SPY),
		"lookback"
	)

	testthat::expect_equal(
		object = output,
		expected = lookback(FUN = normalized_average_true_range, x = SPY)
	)
...
testthat::test_that(desc = '<plotly>-methods for <data.frame>', code = {
	## check that normalized_average_true_range can
