...
## moving_average_convergence_divergence and MACD produces the same results
testthat::test_that(desc = 'Alias and function similarity', code = {
	...
	camel <- movingAverageConvergenceDivergence(SPY)
...
testthat::test_that(desc = 'Class in, class out (<xts>)', code = {
	...
	##    matches the input class
	testthat::expect_true(
		inherits(moving_average_convergence_divergence(GOOGL), class(GOOGL))
	)
})
...
testthat::test_that(desc = 'Value parity with <data.frame> (<xts>)', code = {
	...
	reference <- unname(as.matrix(moving_average_convergence_divergence(
		reference
	)))

	testthat::expect_equal(
		object = unname(zoo::coredata(moving_average_convergence_divergence(
			GOOGL
		))),
		expected = reference
	)
	...
	testthat::expect_equal(
		object = unname(zoo::coredata(moving_average_convergence_divergence(GOOGL[, sort(colnames(
			GOOGL
		))]))),
		expected = reference
	)
...
testthat::test_that(desc = 'Equal length of input and output for <data.frame> with na.bridge = TRUE', code = {
	testthat::expect_equal(
		object = nrow(moving_average_convergence_divergence(
			ATOM,
			na.bridge = TRUE
		)),
		expected = nrow(ATOM)
...
testthat::test_that(desc = 'Row names are respected for <data.frame>, na.bridge = TRUE', code = {
	...
	## calculate indicator
	indicator <- moving_average_convergence_divergence(ATOM, na.bridge = TRUE)
...
testthat::test_that(desc = 'Row names are respected for <data.frame>', code = {
	...
	## calculate indicator
	indicator <- moving_average_convergence_divergence(BTC)
...
testthat::test_that(desc = 'Row names are respected for <matrix>', code = {
	...
	## calculate indicator
	indicator <- moving_average_convergence_divergence(SPY)
...
testthat::test_that(desc = 'Lookback equivalence', code = {
	output <- attr(
		moving_average_convergence_divergence(x = SPY),
		"lookback"
	)
	...
	testthat::expect_identical(
		object = talib:::movingAverageConvergenceDivergence_lookback,
		expected = talib:::moving_average_convergence_divergence_lookback
	)
})
...
testthat::test_that(desc = '<plotly>-methods for <data.frame>', code = {
	## check that moving_average_convergence_divergence can
