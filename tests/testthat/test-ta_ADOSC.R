...
testthat::test_that(desc = 'Alias and function similarity', code = {
	...
	camel <- chaikinAccumulationDistributionOscillator(SPY)
...
testthat::test_that(desc = 'Class in, class out (<data.frame>)', code = {
	...
	##    matches the input class
	testthat::expect_true(
		inherits(chaikin_accumulation_distribution_oscillator(BTC), class(BTC))
	)
})
...
testthat::test_that(desc = 'Class in, class out (<xts>)', code = {
	...
	testthat::expect_true(
		inherits(
			chaikin_accumulation_distribution_oscillator(GOOGL),
			class(GOOGL)
		)
	)
})
...
testthat::test_that(desc = 'Value parity with <data.frame> (<xts>)', code = {
	...
	reference <- unname(as.matrix(chaikin_accumulation_distribution_oscillator(
		reference
	)))

	testthat::expect_equal(
		object = unname(zoo::coredata(chaikin_accumulation_distribution_oscillator(
			GOOGL
		))),
		expected = reference
	)
	...
	testthat::expect_equal(
		object = unname(zoo::coredata(chaikin_accumulation_distribution_oscillator(GOOGL[, sort(colnames(
			GOOGL
		))]))),
		expected = reference
	)
...
testthat::test_that(desc = 'Lookback equivalence', code = {
	...
	testthat::expect_identical(
		object = talib:::chaikinAccumulationDistributionOscillator_lookback,
...
testthat::test_that(desc = '<plotly>-methods for <data.frame>', code = {
	## check that chaikin_accumulation_distribution_oscillator can
