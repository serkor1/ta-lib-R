...
testthat::test_that(desc = 'Value parity with <data.frame> (<xts>)', code = {
	...
	reference <- unname(as.matrix(variable_moving_average_period(
		reference,
		periods = rep(5, nrow(GOOGL))
	)))

	testthat::expect_equal(
		object = unname(zoo::coredata(variable_moving_average_period(
			GOOGL,
			periods = rep(5, nrow(GOOGL))
		))),
		expected = reference
...
testthat::test_that(desc = 'Lookback equivalence', code = {
	...
	testthat::expect_equal(
		object = output,
		expected = lookback(
			FUN = variable_moving_average_period,
			x = SPY,
			periods = rep(5, nrow(SPY))
		)
	)
	...
	testthat::expect_identical(
		object = talib:::variableMovingAveragePeriod_lookback,
		expected = talib:::variable_moving_average_period_lookback
	)
})
