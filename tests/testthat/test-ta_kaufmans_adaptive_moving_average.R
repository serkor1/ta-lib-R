...
##    kaufmans_adaptive_moving_average and KAMA produces the same results
...
testthat::test_that(desc = '{plotly}-methods for <data.frame>', code = {
	## 1) check that kaufmans_adaptive_moving_average can
...
testthat::test_that(desc = 'Equal length of input and output', code = {
	testthat::expect_equal(
		object = nrow(kaufmans_adaptive_moving_average(
			BTC
		)),
		expected = nrow(BTC)
	)
