...
## commodity_channel_index and CCI produces the same results
testthat::test_that(desc = 'Alias and function similarity', code = {
	...
	camel <- commodityChannelIndex(SPY)
...
testthat::test_that(desc = 'Class in, class out (<xts>)', code = {
	...
	##    matches the input class
	testthat::expect_true(
		inherits(commodity_channel_index(GOOGL), class(GOOGL))
	)
})
...
testthat::test_that(desc = 'Value parity with <data.frame> (<xts>)', code = {
	...
	reference <- unname(as.matrix(commodity_channel_index(reference)))

	testthat::expect_equal(
		object = unname(zoo::coredata(commodity_channel_index(GOOGL))),
		expected = reference
	)
	...
	testthat::expect_equal(
		object = unname(zoo::coredata(commodity_channel_index(GOOGL[, sort(colnames(
			GOOGL
		))]))),
		expected = reference
	)
...
testthat::test_that(desc = 'Equal length of input and output for <data.frame> with na.bridge = TRUE', code = {
	testthat::expect_equal(
		object = nrow(commodity_channel_index(
			ATOM,
			na.bridge = TRUE
		)),
		expected = nrow(ATOM)
...
testthat::test_that(desc = 'Row names are respected for <data.frame>, na.bridge = TRUE', code = {
	...
	## calculate indicator
	indicator <- commodity_channel_index(ATOM, na.bridge = TRUE)
...
testthat::test_that(desc = 'Row names are respected for <data.frame>', code = {
	...
	## calculate indicator
	indicator <- commodity_channel_index(BTC)
...
testthat::test_that(desc = 'Row names are respected for <matrix>', code = {
	...
	## calculate indicator
	indicator <- commodity_channel_index(SPY)
...
testthat::test_that(desc = 'Lookback equivalence', code = {
	output <- attr(
		commodity_channel_index(x = SPY),
		"lookback"
	)

	testthat::expect_equal(
		object = output,
		expected = lookback(FUN = commodity_channel_index, x = SPY)
	)

	## the camelCase lookback alias
	## is the function itself
	testthat::expect_identical(
		object = talib:::commodityChannelIndex_lookback,
		expected = talib:::commodity_channel_index_lookback
	)
})
...
testthat::test_that(desc = '<plotly>-methods for <data.frame>', code = {
	## check that commodity_channel_index can
