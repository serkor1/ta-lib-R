...
testthat::test_that(desc = 'Lookback equivalence', code = {
	output <- attr(
		rolling_minimum(x = SPY[, 1]),
		"lookback"
	)

	testthat::expect_equal(
		object = output,
		expected = lookback(FUN = rolling_minimum, x = SPY[, 1])
	)
})
...
testthat::test_that(desc = 'Multivariate input errors', code = {
	...
	testthat::expect_error(rolling_minimum(x = GOOGL))
	testthat::expect_error(rolling_minimum(x = SPY))
})
