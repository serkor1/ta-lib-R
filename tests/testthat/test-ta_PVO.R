...
testthat::test_that(desc = 'Alias and function similarity', code = {
	...
	camel <- percentageVolumeOscillator(SPY)
...
testthat::test_that(desc = 'Class in, class out (<matrix>)', code = {
	...
	testthat::expect_true(
		inherits(percentage_volume_oscillator(SPY), class(SPY))
	)
})
...
testthat::test_that(desc = 'Class in, class out (<data.frame>)', code = {
	...
	##    matches the input class
	testthat::expect_true(
		inherits(percentage_volume_oscillator(BTC), class(BTC))
	)
})
...
testthat::test_that(desc = 'Class in, class out (<xts>)', code = {
	...
	##    matches the input class
	testthat::expect_true(
		inherits(percentage_volume_oscillator(GOOGL), class(GOOGL))
	)
})
...
testthat::test_that(desc = 'Value parity with <data.frame> (<xts>)', code = {
	...
	reference <- unname(as.matrix(percentage_volume_oscillator(reference)))

	testthat::expect_equal(
		object = unname(zoo::coredata(percentage_volume_oscillator(GOOGL))),
		expected = reference
	)
	...
	testthat::expect_equal(
		object = unname(zoo::coredata(percentage_volume_oscillator(GOOGL[, sort(colnames(
			GOOGL
		))]))),
		expected = reference
	)
...
testthat::test_that(desc = 'Default calls', code = {
	testthat::expect_equal(
		object = percentage_volume_oscillator(
			BTC
		),
		expected = percentage_volume_oscillator(
			BTC,
			cols = ~volume
		)
	)
...
testthat::test_that(desc = 'Equal length of input and output for <data.frame> with na.bridge = TRUE', code = {
	testthat::expect_equal(
		object = nrow(percentage_volume_oscillator(
			ATOM,
			na.bridge = TRUE
		)),
		expected = nrow(ATOM)
...
testthat::test_that(desc = 'Row names are respected for <data.frame>, na.bridge = TRUE', code = {
	...
	## calculate indicator
	indicator <- percentage_volume_oscillator(ATOM, na.bridge = TRUE)
...
testthat::test_that(desc = 'Row names are respected for <data.frame>', code = {
	...
	## calculate indicator
	indicator <- percentage_volume_oscillator(BTC)
...
testthat::test_that(desc = 'Row names are respected for <matrix>', code = {
	...
	## calculate indicator
	indicator <- percentage_volume_oscillator(SPY)
...
testthat::test_that(desc = 'Lookback equivalence', code = {
	output <- attr(
		percentage_volume_oscillator(x = SPY),
		"lookback"
	)

	testthat::expect_equal(
		object = output,
		expected = lookback(FUN = percentage_volume_oscillator, x = SPY)
	)

	## the camelCase lookback alias
	## is the function itself
	testthat::expect_identical(
		object = talib:::percentageVolumeOscillator_lookback,
		expected = talib:::percentage_volume_oscillator_lookback
	)
})
...
testthat::test_that(desc = '<plotly>-methods for <data.frame>', code = {
	## check that percentage_volume_oscillator can
	## use <plotly> without any issues
	output <- testthat::expect_no_error(
		{
			chart(BTC)
			indicator(percentage_volume_oscillator)
		}
	)
...
testthat::test_that(desc = '<numeric> methods', code = {
	...
	x <- testthat::expect_no_condition(
		percentage_volume_oscillator(BTC[[1]])
	)
