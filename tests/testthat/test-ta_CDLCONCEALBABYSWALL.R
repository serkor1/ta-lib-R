...
## concealing_baby_swallow and CDLCONCEALBABYSWALL produces the same results
testthat::test_that(desc = 'Alias and function similarity', code = {
	## 1) test that the alias and
	##    function returns the same values
	output <- concealing_baby_swallow(SPY)
	alias <- CDLCONCEALBABYSWALL(SPY)
	...
	camel <- concealingBabySwallow(SPY)
...
testthat::test_that(desc = 'Class in, class out (<matrix>)', code = {
	...
	testthat::expect_true(
		inherits(concealing_baby_swallow(SPY), class(SPY))
	)
})
...
testthat::test_that(desc = 'Class in, class out (<data.frame>)', code = {
	...
	##    matches the input class
	testthat::expect_true(
		inherits(concealing_baby_swallow(BTC), class(BTC))
	)
})
...
testthat::test_that(desc = 'Class in, class out (<xts>)', code = {
	...
	##    matches the input class
	testthat::expect_true(
		inherits(concealing_baby_swallow(GOOGL), class(GOOGL))
	)
})
...
testthat::test_that(desc = 'Value parity with <data.frame> (<xts>)', code = {
	...
	reference <- unname(as.matrix(concealing_baby_swallow(reference)))

	testthat::expect_equal(
		object = unname(zoo::coredata(concealing_baby_swallow(GOOGL))),
		expected = reference
	)
	...
	testthat::expect_equal(
		object = unname(zoo::coredata(concealing_baby_swallow(GOOGL[, sort(colnames(
			GOOGL
		))]))),
		expected = reference
	)
...
testthat::test_that(desc = 'Default calls', code = {
	testthat::expect_equal(
		object = concealing_baby_swallow(
			BTC
		),
		expected = concealing_baby_swallow(
			BTC,
...
testthat::test_that(desc = 'Equal length of input and output for <data.frame> with na.bridge = TRUE', code = {
	testthat::expect_equal(
		object = nrow(concealing_baby_swallow(
			ATOM,
			na.bridge = TRUE
		)),
		expected = nrow(ATOM)
...
testthat::test_that(desc = 'Row names are respected for <data.frame>, na.bridge = TRUE', code = {
	...
	## calculate indicator
	indicator <- concealing_baby_swallow(ATOM, na.bridge = TRUE)
...
testthat::test_that(desc = 'Row names are respected for <data.frame>', code = {
	...
	## calculate indicator
	indicator <- concealing_baby_swallow(BTC)
...
testthat::test_that(desc = 'Row names are respected for <matrix>', code = {
	...
	## calculate indicator
	indicator <- concealing_baby_swallow(SPY)
...
testthat::test_that(desc = 'Lookback equivalence', code = {
	output <- attr(
		concealing_baby_swallow(x = SPY),
		"lookback"
	)

	testthat::expect_equal(
		object = output,
		expected = lookback(FUN = concealing_baby_swallow, x = SPY)
	)

	## the camelCase lookback alias
	## is the function itself
	testthat::expect_identical(
		object = talib:::concealingBabySwallow_lookback,
		expected = talib:::concealing_baby_swallow_lookback
	)
})
...
testthat::test_that(desc = '<plotly>-methods for <data.frame>', code = {
	## check that concealing_baby_swallow can
	## use <plotly> without any issues
	output <- testthat::expect_no_error(
		{
			chart(BTC)
			indicator(concealing_baby_swallow)
		}
	)
