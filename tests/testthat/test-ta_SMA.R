...
testthat::test_that(desc = "Simple Moving Average", code = {
	...
	## 3.1) matrix
	testthat::expect_true(
		inherits(simple_moving_average(SPY), class(SPY))
	)

	## 3.2) data.frame
	testthat::expect_true(
		inherits(simple_moving_average(BTC), class(BTC))
	)

	## 3.3) vectors
	testthat::expect_true(
		is.vector(simple_moving_average(BTC[, 1]))
	)

	## 4) test that default
	##    values equals
	testthat::expect_equal(
		object = simple_moving_average(
			BTC
		),
