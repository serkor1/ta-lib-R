...
testthat::test_that(desc = 'Alias and function similarity', code = {
	...
	camel <- absolutePriceOscillator(SPY)
...
testthat::test_that(desc = 'Class in, class out (<matrix>)', code = {
	...
	testthat::expect_true(
		inherits(absolute_price_oscillator(SPY), class(SPY))
	)
})
...
testthat::test_that(desc = 'Class in, class out (<data.frame>)', code = {
	...
	##    matches the input class
	testthat::expect_true(
		inherits(absolute_price_oscillator(BTC), class(BTC))
	)
})
...
testthat::test_that(desc = 'Value parity with <data.frame> (<xts>)', code = {
	...
	reference <- unname(as.matrix(absolute_price_oscillator(reference)))

	testthat::expect_equal(
		object = unname(zoo::coredata(absolute_price_oscillator(GOOGL))),
		expected = reference
	)
	...
	testthat::expect_equal(
		object = unname(zoo::coredata(absolute_price_oscillator(GOOGL[, sort(colnames(
			GOOGL
		))]))),
		expected = reference
	)
...
testthat::test_that(desc = 'Default calls', code = {
	testthat::expect_equal(
		object = absolute_price_oscillator(
			BTC
		),
		expected = absolute_price_oscillator(
			BTC,
			cols = ~close
		)
	)
...
testthat::test_that(desc = 'Equal length of input and output for <data.frame> with na.bridge = TRUE', code = {
	testthat::expect_equal(
		object = nrow(absolute_price_oscillator(
			ATOM,
			na.bridge = TRUE
		)),
		expected = nrow(ATOM)
...
testthat::test_that(desc = 'Row names are respected for <data.frame>, na.bridge = TRUE', code = {
	...
	## calculate indicator
	indicator <- absolute_price_oscillator(ATOM, na.bridge = TRUE)
...
testthat::test_that(desc = 'Row names are respected for <data.frame>', code = {
	...
	## calculate indicator
	indicator <- absolute_price_oscillator(BTC)
...
testthat::test_that(desc = 'Row names are respected for <matrix>', code = {
	...
	## calculate indicator
	indicator <- absolute_price_oscillator(SPY)
...
testthat::test_that(desc = 'Lookback equivalence', code = {
	output <- attr(
		absolute_price_oscillator(x = SPY),
		"lookback"
	)

	testthat::expect_equal(
		object = output,
		expected = lookback(FUN = absolute_price_oscillator, x = SPY)
	)
...
testthat::test_that(desc = '<plotly>-methods for <data.frame>', code = {
	## check that absolute_price_oscillator can
