...
The function returns the required lookback for calculating the
indicator. A lookback of 0 means the indicator is computable from the
first observation
([`talib::balance_of_power()`](https://serkor1.github.io/ta-lib-R/reference/balance_of_power.md),
for example). Its use-case is customized control-flows for downstream
wrappers and/or packages that declares dependency on {talib}.

- Each calculated indicator carries a `lookback` attribute: the total
  number of leading rows that are not computed. The attribute is
  cumulative, so chaining indicators sums each stage’s lookback, and it
  is used internally to chain indicators deterministically at full input
  length:
...
The chain carries RSI (14), SMA (29) and BBANDS
(19)—`attr(output, "lookback")` is their sum, the leading-NA head of the
final series. The per-indicator lookback remains available via
[`talib::lookback()`](https://serkor1.github.io/ta-lib-R/reference/lookback.md).
