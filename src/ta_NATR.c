// interface to ta_NATR.c
//
// Description
//   R wrapper for TA_NATR. Computes Normalized Average True Range (%).
//
// Parameters
...
//
// Returns
//   n x 1 REAL matrix with column "natr", padded with NA_REAL for
//   the initial lookback. NATR has an unstable period in TA-Lib.
//
// References
...
// clang-format off
SEXP impl_ta_NATR(SEXP high, SEXP low, SEXP close, SEXP timeperiod) {
  // clang-format on
  int protect_count = 0;
  ...
  const int minimum_lookback = TA_NATR_Lookback(period);
  ...
  if (n < minimum_lookback) {
    ...
    // clang-format off
    TA_RetCode return_code = TA_NATR(
      /*startIdx*/ 0,
      /*endIdx  */ n - 1,
      /*inHigh  */ high_ptr,
      /*inLow   */ low_ptr,
    ...
    if (return_code != TA_SUCCESS) {
      ...
      Rf_error("TA_NATR failed: return code %d", return_code);
    }

    shift_array(out_ptr, n, out_begin);
    set_colnames(result, "NATR");
  }
