...
//
// Source
//      https://github.com/TA-Lib/ta-lib/blob/main/src/ta_func/ta_CDLDOJISTAR.c
//
...
// clang-format off
SEXP impl_ta_CDLDOJISTAR_lookback(
    SEXP inOpen,
    SEXP inHigh,
    SEXP inLow,
    SEXP inClose
...
// clang-format on
{
  ...
  // calculate lookback
  const int lookback = TA_CDLDOJISTAR_Lookback();
...
// clang-format on
{
  ...
  if (proceed) {
    ...
    // TA_CDLDOJISTAR returns an TA_RetCode
  ...
  set_colnames(output, "CDLDOJISTAR");
  ...
  // ta_CDLDOJISTAR returns values in the range [-200, 200]
