...
//
// Description
// Identifies the Rickshaw Man candlestick pattern (a long-legged doji variant).
...
//
// Returns
// Integer vector length n. Rickshaw Man is neutral/agnostic.
...
SEXP impl_ta_CDLRICKSHAWMAN(
  ...
  const int minimum_lookback = TA_CDLRICKSHAWMAN_Lookback();
  ...
  if (n < minimum_lookback) {
    ...
    if (return_code != TA_SUCCESS) {
      ...
      Rf_error("TA_CDLRICKSHAWMAN failed: %d", return_code);
    }
