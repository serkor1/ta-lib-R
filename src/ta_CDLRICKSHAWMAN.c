// interface to ta_CDLRICKSHAWMAN.c
//
// Parameters
//      double  inOpen
//      double  inClose
...
//
// Source
//      https://github.com/TA-Lib/ta-lib/blob/main/src/ta_func/ta_CDLRICKSHAWMAN.c
//
...
// clang-format off
SEXP impl_ta_CDLRICKSHAWMAN_lookback(
    SEXP inOpen,
    SEXP inHigh,
    SEXP inLow,
    SEXP inClose
...
// clang-format on
{
  ...
  // calculate lookback
  const int lookback = TA_CDLRICKSHAWMAN_Lookback();
...
// clang-format on
{
  ...
  if (proceed) {
    ...
    // TA_CDLRICKSHAWMAN returns an TA_RetCode
    // which is TA_SUCCESS if it succeeds
    // values in output_ptr gets populated
    // by pointers
    TA_RetCode return_code = TA_CDLRICKSHAWMAN(
      0,
      n - 1,
      open_ptr,
      high_ptr,
  ...
  set_colnames(output, "CDLRICKSHAWMAN");
  ...
  // ta_CDLRICKSHAWMAN returns values in the range [-200, 200]
