...
//
// Source
//      https://github.com/TA-Lib/ta-lib/blob/main/src/ta_func/ta_CDLSHORTLINE.c
//
...
// clang-format off
SEXP impl_ta_CDLSHORTLINE_lookback(
    SEXP inOpen,
    SEXP inHigh,
    SEXP inLow,
    SEXP inClose
...
// clang-format on
{
  ...
  // calculate lookback
  const int lookback = TA_CDLSHORTLINE_Lookback();
...
// clang-format on
{
  ...
  if (proceed) {
    ...
    // TA_CDLSHORTLINE returns an TA_RetCode
  ...
  set_colnames(output, "CDLSHORTLINE");
  ...
  // ta_CDLSHORTLINE returns values in the range [-200, 200]
