...
//
// Source
//      https://github.com/TA-Lib/ta-lib/blob/main/src/ta_func/ta_CDLKICKINGBYLENGTH.c
//
#include "MAType.h"
...
// clang-format off
SEXP impl_ta_CDLKICKINGBYLENGTH_lookback(
	SEXP inOpen,
	SEXP inHigh,
	SEXP inLow,
	SEXP inClose
...
// clang-format on
{
  ...
  // calculate lookback
  const int lookback = TA_CDLKICKINGBYLENGTH_Lookback();
...
// clang-format on
{
  ...
  if (proceed) {
    ...
    // TA_CDLKICKINGBYLENGTH returns an TA_RetCode
  ...
  set_colnames(output, "CDLKICKINGBYLENGTH");
  ...
  // ta_CDLKICKINGBYLENGTH returns values in the range [-200, 200]
