...
//
// Source
//      https://github.com/TA-Lib/ta-lib/blob/main/src/ta_func/ta_MAMA.c
//
#include "MAType.h"
...
// clang-format on
{
  ...
  // calculate lookback
  const int lookback =
    TA_MAMA_Lookback(optInFastLimit_value, optInSlowLimit_value);
...
// clang-format on
{
  ...
  if (proceed) {
    ...
    double *mama = output_ptr;
    double *fama = output_ptr + 1 * n;

    // TA_MAMA returns an TA_RetCode
    // which is TA_SUCCESS if it succeeds
    // values in output_ptr gets populated
    // by pointers
    TA_RetCode return_code = TA_MAMA(
      0,
      n - 1,
      inReal_ptr,
      optInFastLimit_value,
      optInSlowLimit_value,
      &start_idx,
      &end_idx,
      mama,
      fama);
