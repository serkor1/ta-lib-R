...
//
// Source
//      https://github.com/TA-Lib/ta-lib/blob/main/src/ta_func/ta_CDLMATCHINGLOW.c
//
...
// clang-format off
SEXP impl_ta_CDLMATCHINGLOW_lookback(
    SEXP inOpen,
    SEXP inHigh,
    SEXP inLow,
    SEXP inClose
...
// clang-format on
{
  ...
  // calculate lookback
  const int lookback = TA_CDLMATCHINGLOW_Lookback();
...
// clang-format on
{
  ...
  if (proceed) {
    ...
    // TA_CDLMATCHINGLOW returns an TA_RetCode
  ...
  set_colnames(output, "CDLMATCHINGLOW");
  ...
  // ta_CDLMATCHINGLOW returns values in the range [-200, 200]
