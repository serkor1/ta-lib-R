...
// clang-format off
SEXP impl_ta_CDLMORNINGDOJISTAR_lookback(
	SEXP inOpen,
	SEXP inHigh,
	SEXP inLow,
	SEXP inClose,
...
// clang-format on
{
  ...
  // calculate lookback
  const int lookback = TA_CDLMORNINGDOJISTAR_Lookback(optInPenetration_value);
...
// clang-format on
{
  ...
  // ta_CDLMORNINGDOJISTAR returns values in the range [-200, 200]
