...
//
// Description
// Identifies the On-Neck candlestick pattern.
...
//
// Returns
// Integer vector length n. On-Neck is a bearish continuation pattern.
...
SEXP impl_ta_CDLONNECK(
  ...
  const int minimum_lookback = TA_CDLONNECK_Lookback();
  ...
  if (n < minimum_lookback) {
    ...
    if (return_code != TA_SUCCESS) {
      ...
      Rf_error("TA_CDLONNECK failed: %d", return_code);
    }
