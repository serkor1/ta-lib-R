...
//
// Source
//      https://github.com/TA-Lib/ta-lib/blob/main/src/ta_func/ta_CDLONNECK.c
//
...
// clang-format off
SEXP impl_ta_CDLONNECK_lookback(
    SEXP inOpen,
    SEXP inHigh,
    SEXP inLow,
    SEXP inClose
...
// clang-format on
{
  ...
  // calculate lookback
  const int lookback = TA_CDLONNECK_Lookback();
...
// clang-format on
{
  ...
  if (proceed) {
    ...
    // TA_CDLONNECK returns an TA_RetCode
    // which is TA_SUCCESS if it succeeds
    // values in output_ptr gets populated
    // by pointers
    TA_RetCode return_code = TA_CDLONNECK(
      0,
      n - 1,
      open_ptr,
      high_ptr,
  ...
  // ta_CDLONNECK returns values in the range [-200, 200]
