...
//
// Source
//      https://github.com/TA-Lib/ta-lib/blob/main/src/ta_func/ta_CDLMARUBOZU.c
//
...
// clang-format off
SEXP impl_ta_CDLMARUBOZU_lookback(
    SEXP inOpen,
    SEXP inHigh,
    SEXP inLow,
    SEXP inClose
...
// clang-format on
{
  ...
  // calculate lookback
  const int lookback = TA_CDLMARUBOZU_Lookback();
...
// clang-format on
{
  ...
  if (proceed) {
    ...
    // TA_CDLMARUBOZU returns an TA_RetCode
  ...
  set_colnames(output, "CDLMARUBOZU");
  ...
  // ta_CDLMARUBOZU returns values in the range [-200, 200]
