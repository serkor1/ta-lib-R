...
// clang-format on
{
  ...
  const double optInVFactor_value = REAL(optInVFactor)[0];

  // calculate lookback
  const int lookback =
    TA_T3_Lookback(optInTimePeriod_value, optInVFactor_value);
