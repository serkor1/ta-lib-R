...
//
// Description
// R C-interface for TA-Lib's CDLUNIQUE3RIVER. Detects Unique Three River.
//
// Parameters
...
//
// Returns
// Integer vector length n. Pattern is typically bullish reversal: +100 on
...
SEXP impl_ta_CDLUNIQUE3RIVER(SEXP open, SEXP high, SEXP low, SEXP close,
                             SEXP normalize_flag) {
  ...
  const int minimum_lookback = TA_CDLUNIQUE3RIVER_Lookback();
  ...
  if (n < minimum_lookback) {
    ...
    if (return_code != TA_SUCCESS) {
      ...
      Rf_error("TA_CDLUNIQUE3RIVER failed: %d", return_code);
    }
