// interface to ta_CDLUNIQUE3RIVER.c
//
// Parameters
// 		double  inOpen
// 		double  inHigh
...
//
// Source
//      https://github.com/TA-Lib/ta-lib/blob/main/src/ta_func/ta_CDLUNIQUE3RIVER.c
//
#include "MAType.h"
...
// clang-format off
SEXP impl_ta_CDLUNIQUE3RIVER_lookback(
	SEXP inOpen,
	SEXP inHigh,
	SEXP inLow,
	SEXP inClose
...
// clang-format on
{
  ...
  // calculate lookback
  const int lookback = TA_CDLUNIQUE3RIVER_Lookback();
...
// clang-format on
{
  ...
  if (proceed) {
    ...
    // TA_CDLUNIQUE3RIVER returns an TA_RetCode
    // which is TA_SUCCESS if it succeeds
    // values in output_ptr gets populated
    // by pointers
    TA_RetCode return_code = TA_CDLUNIQUE3RIVER(
      0,
      n - 1,
      inOpen_ptr,
      inHigh_ptr,
  ...
  set_colnames(output, "CDLUNIQUE3RIVER");
  ...
  // ta_CDLUNIQUE3RIVER returns values in the range [-200, 200]
