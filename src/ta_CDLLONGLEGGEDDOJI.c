// interface to ta_CDLLONGLEGGEDDOJI.c
//
// Parameters
// 		double  inOpen
// 		double  inHigh
...
//
// Source
//      https://github.com/TA-Lib/ta-lib/blob/main/src/ta_func/ta_CDLLONGLEGGEDDOJI.c
//
#include "MAType.h"
...
// clang-format off
SEXP impl_ta_CDLLONGLEGGEDDOJI_lookback(
	SEXP inOpen,
	SEXP inHigh,
	SEXP inLow,
	SEXP inClose
...
// clang-format on
{
  ...
  // calculate lookback
  const int lookback = TA_CDLLONGLEGGEDDOJI_Lookback();
...
// clang-format off
SEXP impl_ta_CDLLONGLEGGEDDOJI(
	SEXP inOpen,
	SEXP inHigh,
	SEXP inLow,
	SEXP inClose,
...
// clang-format on
{
  ...
  if (proceed) {
    ...
    // TA_CDLLONGLEGGEDDOJI returns an TA_RetCode
    // which is TA_SUCCESS if it succeeds
    // values in output_ptr gets populated
    // by pointers
    TA_RetCode return_code = TA_CDLLONGLEGGEDDOJI(
      0,
      n - 1,
      inOpen_ptr,
      inHigh_ptr,
  ...
  set_colnames(output, "CDLLONGLEGGEDDOJI");
  ...
  // ta_CDLLONGLEGGEDDOJI returns values in the range [-200, 200]
