// interface to ta_CDLMORNINGSTAR.c
//
// Description
// Identifies the Morning Star candlestick pattern. Optional penetration factor.
// Values are {-100, 0, 100}; optionally normalized to {-1, 0, 1}.
//
// Parameters
// open, high, low, close: numeric vectors
// penetration: numeric scalar in [0,1]
// normalize_flag: logical
//
// Returns
// Integer vector length n. Morning Star is a bullish reversal pattern.
...
SEXP impl_ta_CDLMORNINGSTAR(
  ...
  if (n < minimum_lookback) {
    ...
    if (return_code != TA_SUCCESS) {
      ...
      Rf_error("TA_CDLMORNINGSTAR failed: %d", return_code);
    }
