...
// clang-format off
SEXP impl_ta_CDLMORNINGSTAR_lookback(
	SEXP inOpen,
	SEXP inHigh,
	SEXP inLow,
	SEXP inClose,
...
// clang-format on
{
  ...
  // calculate lookback
  const int lookback = TA_CDLMORNINGSTAR_Lookback(optInPenetration_value);
...
// clang-format on
{
  ...
  if (proceed) {
    ...
    // TA_CDLMORNINGSTAR returns an TA_RetCode
  ...
  // ta_CDLMORNINGSTAR returns values in the range [-200, 200]
