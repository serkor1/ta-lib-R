// interface to ta_CDLDRAGONFLYDOJI.c
//
// Parameters
//      double  inOpen
//      double  inClose
...
//
// Source
//      https://github.com/TA-Lib/ta-lib/blob/main/src/ta_func/ta_CDLDRAGONFLYDOJI.c
//
...
// clang-format off
SEXP impl_ta_CDLDRAGONFLYDOJI_lookback(
    SEXP inOpen,
    SEXP inHigh,
    SEXP inLow,
    SEXP inClose
...
// clang-format on
{
  ...
  // calculate lookback
  const int lookback = TA_CDLDRAGONFLYDOJI_Lookback();
...
// clang-format on
{
  ...
  if (proceed) {
    ...
    // TA_CDLDRAGONFLYDOJI returns an TA_RetCode
  ...
  set_colnames(output, "CDLDRAGONFLYDOJI");
  ...
  // ta_CDLDRAGONFLYDOJI returns values in the range [-200, 200]
