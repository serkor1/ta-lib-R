...
//
// Description
// Identifies the Mat Hold candlestick pattern. Optional penetration factor.
...
//
// Returns
// Integer vector length n. Mat Hold is a bullish continuation pattern.
...
SEXP impl_ta_CDLMATHOLD(
  ...
  const int minimum_lookback = TA_CDLMATHOLD_Lookback(opt_penetration);
  ...
  if (n < minimum_lookback) {
    ...
    if (return_code != TA_SUCCESS) {
      ...
      Rf_error("TA_CDLMATHOLD failed: %d", return_code);
    }
