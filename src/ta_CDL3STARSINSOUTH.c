// interface to ta_CDL3STARSINSOUTH.c
//
// Parameters
//      double  inOpen
//      double  inClose
...
//
// Source
//      https://github.com/TA-Lib/ta-lib/blob/main/src/ta_func/ta_CDL3STARSINSOUTH.c
//
...
// clang-format off
SEXP impl_ta_CDL3STARSINSOUTH_lookback(
    SEXP inOpen,
    SEXP inHigh,
    SEXP inLow,
    SEXP inClose
...
// clang-format on
{
  ...
  // calculate lookback
  const int lookback = TA_CDL3STARSINSOUTH_Lookback();
...
// clang-format on
{
  ...
  if (proceed) {
    ...
    // TA_CDL3STARSINSOUTH returns an TA_RetCode
    // which is TA_SUCCESS if it succeeds
    // values in output_ptr gets populated
    // by pointers
    TA_RetCode return_code = TA_CDL3STARSINSOUTH(
      0,
      n - 1,
      open_ptr,
      high_ptr,
  ...
  set_colnames(output, "CDL3STARSINSOUTH");
  ...
  // ta_CDL3STARSINSOUTH returns values in the range [-200, 200]
