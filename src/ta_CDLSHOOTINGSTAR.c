...
//
// Description
// R C-interface for TA-Lib's CDLSHOOTINGSTAR. Detects Shooting Star.
//
// Parameters
...
SEXP impl_ta_CDLSHOOTINGSTAR(SEXP open, SEXP high, SEXP low, SEXP close,
                             SEXP normalize_flag) {
  ...
  const int minimum_lookback = TA_CDLSHOOTINGSTAR_Lookback();
  ...
  if (n < minimum_lookback) {
    ...
    if (return_code != TA_SUCCESS) {
      ...
      Rf_error("TA_CDLSHOOTINGSTAR failed: %d", return_code);
    }
