...
//
// Description
// Identifies the Piercing candlestick pattern.
...
//
// Returns
// Integer vector length n. Piercing is a bullish reversal pattern.
...
SEXP impl_ta_CDLPIERCING(
  ...
  const int minimum_lookback = TA_CDLPIERCING_Lookback();
  ...
  if (n < minimum_lookback) {
    ...
    if (return_code != TA_SUCCESS) {
      ...
      Rf_error("TA_CDLPIERCING failed: %d", return_code);
    }
