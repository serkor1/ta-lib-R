// interface to ta_CDLABANDONEDBABY.c
//
// Parameters
// 		double  inOpen
// 		double  inHigh
...
//
// Source
//      https://github.com/TA-Lib/ta-lib/blob/main/src/ta_func/ta_CDLABANDONEDBABY.c
//
#include "MAType.h"
...
// clang-format off
SEXP impl_ta_CDLABANDONEDBABY_lookback(
	SEXP inOpen,
	SEXP inHigh,
	SEXP inLow,
	SEXP inClose,
...
{
  // values
  const double optInPenetration_value = REAL(optInPenetration)[0];

  // calculate lookback
  const int lookback = TA_CDLABANDONEDBABY_Lookback(optInPenetration_value);
...
// clang-format on
{
  ...
  if (proceed) {
    ...
    // TA_CDLABANDONEDBABY returns an TA_RetCode
    // which is TA_SUCCESS if it succeeds
    // values in output_ptr gets populated
    // by pointers
    TA_RetCode return_code = TA_CDLABANDONEDBABY(
      0,
      n - 1,
      inOpen_ptr,
      inHigh_ptr,
  ...
  set_colnames(output, "CDLABANDONEDBABY");
  ...
  // ta_CDLABANDONEDBABY returns values in the range [-200, 200]
