...
//
// Description
//   Belt-hold candlestick pattern. Returns integer vector of length n.
//
// Implementation notes
//   - Minimal checks: only compare n vs TA_CDLBELTHOLD_Lookback().
//   - Shift in-place using shift_array; optional normalize().
...
SEXP impl_ta_CDLBELTHOLD(SEXP open, SEXP high, SEXP low, SEXP close,
                         SEXP normalize_flag) {
  int protect_count = 0;
  ...
  const int minimum_lookback = TA_CDLBELTHOLD_Lookback();
  ...
  if (n < minimum_lookback) {
    ...
    TA_RetCode return_code =
        TA_CDLBELTHOLD(0, n - 1, open_ptr, high_ptr, low_ptr, close_ptr,
                       &outBeg, &outNb, out_ptr);

    if (return_code != TA_SUCCESS) {
      UNPROTECT(protect_count);
      Rf_error("TA_CDLBELTHOLD failed: return code %d", return_code);
    }

    // shift & optional normalize
