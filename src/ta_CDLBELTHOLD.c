...
//
// Source
//      https://github.com/TA-Lib/ta-lib/blob/main/src/ta_func/ta_CDLBELTHOLD.c
//
...
// clang-format off
SEXP impl_ta_CDLBELTHOLD_lookback(
    SEXP inOpen,
    SEXP inHigh,
    SEXP inLow,
    SEXP inClose
...
// clang-format on
{
  ...
  // calculate lookback
  const int lookback = TA_CDLBELTHOLD_Lookback();
...
// clang-format on
{
  ...
  if (proceed) {
    ...
    // TA_CDLBELTHOLD returns an TA_RetCode
