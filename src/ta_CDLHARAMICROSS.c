...
//
// Source
//      https://github.com/TA-Lib/ta-lib/blob/main/src/ta_func/ta_CDLHARAMICROSS.c
//
...
// clang-format off
SEXP impl_ta_CDLHARAMICROSS_lookback(
    SEXP inOpen,
    SEXP inHigh,
    SEXP inLow,
    SEXP inClose
...
// clang-format on
{
  ...
  // calculate lookback
  const int lookback = TA_CDLHARAMICROSS_Lookback();
...
// clang-format on
{
  ...
  if (proceed) {
    ...
    // TA_CDLHARAMICROSS returns an TA_RetCode
  ...
  set_colnames(output, "CDLHARAMICROSS");
  ...
  // ta_CDLHARAMICROSS returns values in the range [-200, 200]
