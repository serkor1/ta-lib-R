...
//
// Description
//   R wrapper for TA-Lib's CDL High-Wave pattern detector.
...
//   100 when High-Wave is detected, 0 otherwise. Agnostic pattern.
...
SEXP impl_ta_CDLHIGHWAVE(
  ...
  const int minimum_lookback = TA_CDLHIGHWAVE_Lookback();
  ...
  if (n < minimum_lookback) {
    ...
    if (return_code != TA_SUCCESS) {
      ...
      Rf_error("TA_CDLHIGHWAVE failed: return code %d", return_code);
    }
