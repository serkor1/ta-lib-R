...
//
// Source
//      https://github.com/TA-Lib/ta-lib/blob/main/src/ta_func/ta_AROONOSC.c
//
#include "MAType.h"
...
// clang-format off
SEXP impl_ta_AROONOSC_lookback(
	SEXP inHigh,
	SEXP inLow,
...
// clang-format on
{
  ...
  // calculate lookback
  const int lookback = TA_AROONOSC_Lookback(optInTimePeriod_value);
...
// clang-format on
{
  ...
  if (proceed) {
    ...
    // TA_AROONOSC returns an TA_RetCode
