// interface to ta_CDLTAKURI.c
//
// Description
// R C-interface for TA-Lib's CDLTAKURI. Detects Takuri line.
//
// Parameters
...
//
// Returns
// Integer vector length n. Pattern is inherently bullish reversal: +100 on
...
SEXP impl_ta_CDLTAKURI(SEXP open, SEXP high, SEXP low, SEXP close,
                       SEXP normalize_flag) {
  ...
  const int minimum_lookback = TA_CDLTAKURI_Lookback();
  ...
  if (n < minimum_lookback) {
    ...
    // clang-format off
    TA_RetCode return_code = TA_CDLTAKURI(
      /*startIdx*/ 0,
      /*endIdx  */ n - 1,
      /*inOpen  */ open_ptr,
      /*inHigh  */ high_ptr,
    ...
    if (return_code != TA_SUCCESS) {
      ...
      Rf_error("TA_CDLTAKURI failed: %d", return_code);
    }
