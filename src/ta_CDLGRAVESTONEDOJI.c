// Interface to TA_CDLGRAVESTONEDOJI (Gravestone Doji)
...
SEXP impl_ta_CDLGRAVESTONEDOJI(SEXP open, SEXP high, SEXP low, SEXP close,
                               SEXP normalize_flag) {
  int protect_count = 0;
  ...
  const int minimum_lookback = TA_CDLGRAVESTONEDOJI_Lookback();
  ...
  if (n < minimum_lookback) {
    ...
    TA_RetCode return_code =
        TA_CDLGRAVESTONEDOJI(0, n - 1, open_ptr, high_ptr, low_ptr, close_ptr,
                             &outBeg, &outNb, out_ptr);
    if (return_code != TA_SUCCESS) {
      UNPROTECT(protect_count);
      Rf_error("TA_CDLGRAVESTONEDOJI failed: return code %d", return_code);
    }
