...
//
// Source
//      https://github.com/TA-Lib/ta-lib/blob/main/src/ta_func/ta_CDLTRISTAR.c
//
#include "MAType.h"
...
// clang-format off
SEXP impl_ta_CDLTRISTAR_lookback(
	SEXP inOpen,
	SEXP inHigh,
	SEXP inLow,
	SEXP inClose
...
// clang-format on
{
  ...
  // calculate lookback
  const int lookback = TA_CDLTRISTAR_Lookback();
...
// clang-format on
{
  ...
  if (proceed) {
    ...
    // TA_CDLTRISTAR returns an TA_RetCode
  ...
  set_colnames(output, "CDLTRISTAR");
  ...
  // ta_CDLTRISTAR returns values in the range [-200, 200]
