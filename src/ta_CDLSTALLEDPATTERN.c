// interface to ta_CDLSTALLEDPATTERN.c
//
// Parameters
//      double  inOpen
//      double  inClose
...
//
// Source
//      https://github.com/TA-Lib/ta-lib/blob/main/src/ta_func/ta_CDLSTALLEDPATTERN.c
//
...
// clang-format off
SEXP impl_ta_CDLSTALLEDPATTERN_lookback(
    SEXP inOpen,
    SEXP inHigh,
    SEXP inLow,
    SEXP inClose
...
// clang-format on
{
  ...
  // calculate lookback
  const int lookback = TA_CDLSTALLEDPATTERN_Lookback();
...
// clang-format off
SEXP impl_ta_CDLSTALLEDPATTERN(
    SEXP inOpen,
    SEXP inHigh,
    SEXP inLow,
    SEXP inClose,
...
// clang-format on
{
  ...
  if (proceed) {
    ...
    // TA_CDLSTALLEDPATTERN returns an TA_RetCode
    // which is TA_SUCCESS if it succeeds
    // values in output_ptr gets populated
    // by pointers
    TA_RetCode return_code = TA_CDLSTALLEDPATTERN(
      0,
      n - 1,
      open_ptr,
      high_ptr,
  ...
  set_colnames(output, "CDLSTALLEDPATTERN");
  ...
  // ta_CDLSTALLEDPATTERN returns values in the range [-200, 200]
