...
//
// Description
// R C-interface for TA-Lib's CDLSTALLEDPATTERN. Detects Stalled/Deliberation
// pattern.
//
// Parameters
...
//
// Returns
// Integer vector length n. Pattern is typically bearish reversal: -100 on
...
SEXP impl_ta_CDLSTALLEDPATTERN(SEXP open, SEXP high, SEXP low, SEXP close,
                               SEXP normalize_flag) {
  ...
  const int minimum_lookback = TA_CDLSTALLEDPATTERN_Lookback();
  ...
  if (n < minimum_lookback) {
    ...
    if (return_code != TA_SUCCESS) {
      ...
      Rf_error("TA_CDLSTALLEDPATTERN failed: %d", return_code);
    }
