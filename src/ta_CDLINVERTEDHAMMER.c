...
//
// Description
//   R wrapper for TA-Lib's CDL Inverted Hammer pattern detector.
...
//   +100 when Inverted Hammer occurs, 0 otherwise. Bullish pattern.
...
SEXP impl_ta_CDLINVERTEDHAMMER(
  ...
  const int minimum_lookback = TA_CDLINVERTEDHAMMER_Lookback();
  ...
  if (n < minimum_lookback) {
    ...
    if (return_code != TA_SUCCESS) {
      ...
      Rf_error("TA_CDLINVERTEDHAMMER failed: return code %d", return_code);
    }
