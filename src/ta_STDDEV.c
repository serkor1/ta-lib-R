...
//
// Source
//      https://github.com/TA-Lib/ta-lib/blob/main/src/ta_func/ta_STDDEV.c
//
#include "MAType.h"
...
// clang-format on
{
  ...
  const double optInNbDev_value = REAL(optInNbDev)[0];

  // calculate lookback
  const int lookback =
    TA_STDDEV_Lookback(optInTimePeriod_value, optInNbDev_value);
