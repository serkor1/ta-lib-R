// interface to ta_CDLRISEFALL3METHODS.c
//
// Description
// R C-interface for TA-Lib's CDLRISEFALL3METHODS. Detects Rising/Falling Three
// Methods.
//
// Parameters
...
//
// Returns
// Integer vector length n. +100 bullish continuation (Rising 3 Methods), -100
// bearish continuation (Falling 3 Methods), 0 otherwise. If normalized: +1, -1,
// 0. :contentReference[oaicite:12]{index=12}
...
SEXP impl_ta_CDLRISEFALL3METHODS(SEXP open, SEXP high, SEXP low, SEXP close,
                                 SEXP normalize_flag) {
  ...
  const int minimum_lookback = TA_CDLRISEFALL3METHODS_Lookback();
  ...
  if (n < minimum_lookback) {
    ...
    // clang-format off
    TA_RetCode return_code = TA_CDLRISEFALL3METHODS(
      /*startIdx*/ 0,
      /*endIdx  */ n - 1,
      /*inOpen  */ open_ptr,
      /*inHigh  */ high_ptr,
    ...
    if (return_code != TA_SUCCESS) {
      ...
      Rf_error("TA_CDLRISEFALL3METHODS failed: %d", return_code);
    }
