// interface to ta_CDLXSIDEGAP3METHODS.c
//
// Description
// R C-interface for TA-Lib's CDLXSIDEGAP3METHODS. Detects Upside/Downside Gap
// Three Methods.
//
// Parameters
...
//
// Returns
// Integer vector length n. +100 bullish continuation (Upside Gap Three
// Methods), -100 bearish continuation (Downside Gap Three Methods), 0
// otherwise. If normalized: +1, -1, 0. :contentReference[oaicite:0]{index=0}
...
SEXP impl_ta_CDLXSIDEGAP3METHODS(
  ...
  const int minimum_lookback = TA_CDLXSIDEGAP3METHODS_Lookback();
  ...
  if (n < minimum_lookback) {
    ...
    if (return_code != TA_SUCCESS) {
      ...
      Rf_error("TA_CDLXSIDEGAP3METHODS failed: %d", return_code);
    }
