...
// clang-format off
SEXP impl_ta_HT_TRENDLINE_lookback(
	SEXP inReal
)
// clang-format on
{
...
// clang-format on
{
  ...
  if (proceed) {
    ...
    // TA_HT_TRENDLINE returns an TA_RetCode
    // which is TA_SUCCESS if it succeeds
    // values in output_ptr gets populated
    // by pointers
    TA_RetCode return_code =
      TA_HT_TRENDLINE(0, n - 1, inReal_ptr, &start_idx, &end_idx, real);
  ...
  set_colnames(output, "HT_TRENDLINE");
