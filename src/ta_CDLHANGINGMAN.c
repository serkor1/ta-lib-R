// Interface to TA_CDLHANGINGMAN (Hanging Man)
...
SEXP impl_ta_CDLHANGINGMAN(SEXP open, SEXP high, SEXP low, SEXP close,
                           SEXP normalize_flag) {
  int protect_count = 0;
  ...
  const int minimum_lookback = TA_CDLHANGINGMAN_Lookback();
  ...
  if (n < minimum_lookback) {
    ...
    TA_RetCode rc = TA_CDLHANGINGMAN(0, n - 1, open_ptr, high_ptr, low_ptr,
                                     close_ptr, &outBeg, &outNb, out_ptr);
    if (rc != TA_SUCCESS) {
      UNPROTECT(protect_count);
      Rf_error("TA_CDLHANGINGMAN failed: return code %d", rc);
    }
