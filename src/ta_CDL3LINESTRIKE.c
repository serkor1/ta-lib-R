// Interface to TA_CDL3LINESTRIKE (Three-Line Strike)
//
// Parameters
...
//
// Description
//   Identifies Three-Line Strike patterns. Returns an integer vector of length
//   n, padded with NA_INTEGER for the initial lookback. Values are (-100, 0,
//   +100) or (-1, 0, +1) if normalized.
...
SEXP impl_ta_CDL3LINESTRIKE(
  ...
  const int minimum_lookback = TA_CDL3LINESTRIKE_Lookback();
  ...
  if (n < minimum_lookback) {
    ...
    if (rc != TA_SUCCESS) {
      ...
      Rf_error("TA_CDL3LINESTRIKE failed: return code %d", rc);
    }

    // align outputs to length n
    shift_array(out_ptr, n, outBeg);

    // optional normalization to {-1,0,1}
