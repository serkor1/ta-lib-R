...
//
// Description
//   R wrapper for TA-Lib's CDL Hikkake pattern detector.
...
//   +100 bullish Hikkake, -100 bearish Hikkake, 0 otherwise.
...
SEXP impl_ta_CDLHIKKAKE(
  ...
  const int minimum_lookback = TA_CDLHIKKAKE_Lookback();
  ...
  if (n < minimum_lookback) {
    ...
    if (return_code != TA_SUCCESS) {
      ...
      Rf_error("TA_CDLHIKKAKE failed: return code %d", return_code);
    }
