...
//
// Source
//      https://github.com/TA-Lib/ta-lib/blob/main/src/ta_func/ta_CDLTHRUSTING.c
//
...
// clang-format off
SEXP impl_ta_CDLTHRUSTING_lookback(
    SEXP inOpen,
    SEXP inHigh,
    SEXP inLow,
    SEXP inClose
...
// clang-format on
{
  ...
  // calculate lookback
  const int lookback = TA_CDLTHRUSTING_Lookback();
...
// clang-format on
{
  ...
  if (proceed) {
    ...
    // TA_CDLTHRUSTING returns an TA_RetCode
  ...
  set_colnames(output, "CDLTHRUSTING");
  ...
  // ta_CDLTHRUSTING returns values in the range [-200, 200]
