// interface to ta_CDLTHRUSTING.c
//
// Description
// R C-interface for TA-Lib's CDLTHRUSTING. Detects Thrusting pattern.
//
// Parameters
...
//
// Returns
// Integer vector length n. Pattern is inherently bearish continuation: -100 on
...
SEXP impl_ta_CDLTHRUSTING(SEXP open, SEXP high, SEXP low, SEXP close,
                          SEXP normalize_flag) {
  ...
  const int minimum_lookback = TA_CDLTHRUSTING_Lookback();
  ...
  if (n < minimum_lookback) {
    ...
    // clang-format off
    TA_RetCode return_code = TA_CDLTHRUSTING(
      /*startIdx*/ 0,
      /*endIdx  */ n - 1,
      /*inOpen  */ open_ptr,
      /*inHigh  */ high_ptr,
    ...
    if (return_code != TA_SUCCESS) {
      ...
      Rf_error("TA_CDLTHRUSTING failed: %d", return_code);
    }
