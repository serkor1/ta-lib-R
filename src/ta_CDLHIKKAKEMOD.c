...
//
// Description
//   R wrapper for TA-Lib's CDL Hikkake Modified pattern detector.
...
//   +100 bullish Hikkake Modified, -100 bearish Hikkake Modified, 0 otherwise.
...
SEXP impl_ta_CDLHIKKAKEMOD(
  ...
  const int minimum_lookback = TA_CDLHIKKAKEMOD_Lookback();
  ...
  if (n < minimum_lookback) {
    ...
    if (return_code != TA_SUCCESS) {
      ...
      Rf_error("TA_CDLHIKKAKEMOD failed: return code %d", return_code);
    }
