// interface to ta_CDLTASUKIGAP.c
//
// Description
// R C-interface for TA-Lib's CDLTASUKIGAP. Detects Tasuki Gap.
//
// Parameters
...
//
// Returns
// Integer vector length n. +100 bullish continuation (Upside Tasuki Gap), -100
// bearish continuation (Downside Tasuki Gap), 0 otherwise. If normalized: +1,
// -1, 0. :contentReference[oaicite:4]{index=4}
...
SEXP impl_ta_CDLTASUKIGAP(SEXP open, SEXP high, SEXP low, SEXP close,
                          SEXP normalize_flag) {
  ...
  const int minimum_lookback = TA_CDLTASUKIGAP_Lookback();
  ...
  if (n < minimum_lookback) {
    ...
    // clang-format off
    TA_RetCode return_code = TA_CDLTASUKIGAP(
      /*startIdx*/ 0,
      /*endIdx  */ n - 1,
      /*inOpen  */ open_ptr,
      /*inHigh  */ high_ptr,
    ...
    if (return_code != TA_SUCCESS) {
      ...
      Rf_error("TA_CDLTASUKIGAP failed: %d", return_code);
    }
