...
//
// Source
//      https://github.com/TA-Lib/ta-lib/blob/main/src/ta_func/ta_CDLTASUKIGAP.c
//
#include "MAType.h"
...
// clang-format off
SEXP impl_ta_CDLTASUKIGAP_lookback(
	SEXP inOpen,
	SEXP inHigh,
	SEXP inLow,
	SEXP inClose
...
// clang-format on
{
  ...
  // calculate lookback
  const int lookback = TA_CDLTASUKIGAP_Lookback();
...
// clang-format on
{
  ...
  if (proceed) {
    ...
    // TA_CDLTASUKIGAP returns an TA_RetCode
  ...
  set_colnames(output, "CDLTASUKIGAP");
  ...
  // ta_CDLTASUKIGAP returns values in the range [-200, 200]
