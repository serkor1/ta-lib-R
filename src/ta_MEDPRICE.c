...
//
// Source
//      https://github.com/TA-Lib/ta-lib/blob/main/src/ta_func/ta_MEDPRICE.c
//
#include "MAType.h"
...
// clang-format off
SEXP impl_ta_MEDPRICE_lookback(
	SEXP inHigh,
	SEXP inLow
)
// clang-format on
{
  ...
  // calculate lookback
  const int lookback = TA_MEDPRICE_Lookback();
...
// clang-format on
{
  ...
  if (proceed) {
    ...
    // TA_MEDPRICE returns an TA_RetCode
    // which is TA_SUCCESS if it succeeds
    // values in output_ptr gets populated
    // by pointers
    TA_RetCode return_code =
      TA_MEDPRICE(0, n - 1, inHigh_ptr, inLow_ptr, &start_idx, &end_idx, real);
