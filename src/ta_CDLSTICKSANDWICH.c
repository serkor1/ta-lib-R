// interface to ta_CDLSTICKSANDWICH.c
//
// Parameters
// 		double  inOpen
// 		double  inHigh
...
//
// Source
//      https://github.com/TA-Lib/ta-lib/blob/main/src/ta_func/ta_CDLSTICKSANDWICH.c
//
#include "MAType.h"
...
// clang-format off
SEXP impl_ta_CDLSTICKSANDWICH_lookback(
	SEXP inOpen,
	SEXP inHigh,
	SEXP inLow,
	SEXP inClose
...
// clang-format on
{
  ...
  // calculate lookback
  const int lookback = TA_CDLSTICKSANDWICH_Lookback();
...
// clang-format on
{
  ...
  if (proceed) {
    ...
    // TA_CDLSTICKSANDWICH returns an TA_RetCode
    // which is TA_SUCCESS if it succeeds
    // values in output_ptr gets populated
    // by pointers
    TA_RetCode return_code = TA_CDLSTICKSANDWICH(
      0,
      n - 1,
      inOpen_ptr,
      inHigh_ptr,
  ...
  set_colnames(output, "CDLSTICKSANDWICH");
  ...
  // ta_CDLSTICKSANDWICH returns values in the range [-200, 200]
