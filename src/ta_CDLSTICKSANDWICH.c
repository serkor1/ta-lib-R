...
//
// Description
// R C-interface for TA-Lib's CDLSTICKSANDWICH. Detects Stick Sandwich.
//
// Parameters
...
//
// Returns
// Integer vector length n. Pattern is usually bullish reversal: +100 on
...
SEXP impl_ta_CDLSTICKSANDWICH(SEXP open, SEXP high, SEXP low, SEXP close,
                              SEXP normalize_flag) {
  ...
  const int minimum_lookback = TA_CDLSTICKSANDWICH_Lookback();
  ...
  if (n < minimum_lookback) {
    ...
    if (return_code != TA_SUCCESS) {
      ...
      Rf_error("TA_CDLSTICKSANDWICH failed: %d", return_code);
    }
