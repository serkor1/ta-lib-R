...
//
// Description
//   R wrapper for TA-Lib's CDL In-Neck pattern detector.
...
//   -100 when In-Neck occurs, 0 otherwise. Bearish pattern.
...
SEXP impl_ta_CDLINNECK(
  ...
  const int minimum_lookback = TA_CDLINNECK_Lookback();
  ...
  if (n < minimum_lookback) {
    ...
    if (return_code != TA_SUCCESS) {
      ...
      Rf_error("TA_CDLINNECK failed: return code %d", return_code);
    }
