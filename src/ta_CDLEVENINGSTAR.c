...
//
// Source
//      https://github.com/TA-Lib/ta-lib/blob/main/src/ta_func/ta_CDLEVENINGSTAR.c
//
#include "MAType.h"
...
// clang-format off
SEXP impl_ta_CDLEVENINGSTAR_lookback(
	SEXP inOpen,
	SEXP inHigh,
	SEXP inLow,
	SEXP inClose,
...
// clang-format on
{
  ...
  // calculate lookback
  const int lookback = TA_CDLEVENINGSTAR_Lookback(optInPenetration_value);
...
// clang-format on
{
  ...
  if (proceed) {
    ...
    // TA_CDLEVENINGSTAR returns an TA_RetCode
  ...
  set_colnames(output, "CDLEVENINGSTAR");
  ...
  // ta_CDLEVENINGSTAR returns values in the range [-200, 200]
