...
//
// Description
//   R wrapper for TA-Lib's CDL Homing Pigeon pattern detector.
...
//   +100 when Homing Pigeon occurs, 0 otherwise. Bullish pattern.
...
SEXP impl_ta_CDLHOMINGPIGEON(
  ...
  const int minimum_lookback = TA_CDLHOMINGPIGEON_Lookback();
  ...
  if (n < minimum_lookback) {
    ...
    if (return_code != TA_SUCCESS) {
      ...
      Rf_error("TA_CDLHOMINGPIGEON failed: return code %d", return_code);
    }
