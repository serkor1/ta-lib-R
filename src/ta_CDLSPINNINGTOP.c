// interface to ta_CDLSPINNINGTOP.c
//
// Description
// R C-interface for TA-Lib's CDLSPINNINGTOP. Detects Spinning Top.
//
// Parameters
...
//
// Returns
// Integer vector length n. Pattern is agnostic/neutral; TA-Lib encodes candle
// color: +100 white, -100 black, 0 otherwise. If normalized: +1, -1, 0.
...
SEXP impl_ta_CDLSPINNINGTOP(SEXP open, SEXP high, SEXP low, SEXP close,
                            SEXP normalize_flag) {
  ...
  const int minimum_lookback = TA_CDLSPINNINGTOP_Lookback();
  ...
  if (n < minimum_lookback) {
    ...
    // clang-format off
    TA_RetCode return_code = TA_CDLSPINNINGTOP(
      /*startIdx*/ 0,
      /*endIdx  */ n - 1,
      /*inOpen  */ open_ptr,
      /*inHigh  */ high_ptr,
    ...
    if (return_code != TA_SUCCESS) {
      ...
      Rf_error("TA_CDLSPINNINGTOP failed: %d", return_code);
    }
