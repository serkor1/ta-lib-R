...
#' @export
relative_strength_index.plotly <- function(x, n = 10, lower = 20, upper = 80, ...) {
  dots   <- list(...); series <- dots$.series
  rsi    <- .Call("impl_ta_RSI", series, as.integer(n))
  df     <- data.frame(idx = seq_along(rsi), value = rsi)

  rsi_plot <- plotly::plot_ly(df, x = ~idx, y = ~value,
                              type = "scatter", mode = "lines",
                              showlegend = FALSE)
  rsi_plot <- plotly::add_ribbons(
    rsi_plot, x = ~idx,
    ymin = rep(lower, nrow(df)), ymax = rep(upper, nrow(df)),
    line = list(width = 0), fillcolor = "rgba(160,160,160,0.20)"
  )
