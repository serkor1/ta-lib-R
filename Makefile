...
build: ## Build the R package
	@tools/generate_API.sh src/ src/api.h && tools/generate_FFI.sh src/api.h src/init.c && $(MAKE) fmt-default
...
fmt-default: ## Format "api.h" and "init.c". Should only be invoked via $make build.
